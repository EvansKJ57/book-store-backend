...
const addToCart = async (userId, bookId, qty) => {
  ...
  const [results] = await mariadb.execute(insertCartQuery, values);
  return results;
};
...
const deleteCart = async (cartsId, userId, conn) => {
  ...
  if (conn) {
    const [results] = await conn.execute(deleteCartQuery, values);
    return results;
  } else {
    const [results] = await mariadb.execute(deleteCartQuery, values);
    return results;
  }
};
