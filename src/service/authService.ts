...
const AuthService = {
  ...
  reissueAcToken: async (rfToken: string) => {
    try {
  ...
  getAcTokenAndIdTokenFromGoogle: async (code: string) => {
  ...
  checkIdTokenFromGoogle: async (idToken: string, nonce: string) => {
    try {
  ...
  registerOrLoginGoogleUser: async ({
    code,
    nonce,
  }: {
    code: string;
    ...
    try {
      ...
      const { email, name, sub } = await AuthService.checkIdTokenFromGoogle(
